...
const EditThemeModal = ({ isThemeEditModalOpen, close, editId }) => {
  ...
  const [invokeTheme, setinvokeTheme] = useState(true);
  ...
  useEffect(() => {
    const editTheme = async () => {
      ...
      const response = await invokeApi(
        config.baseUrl + apiList.editTheme,
        params,
        cookies
      );
      ...
      if (response && response.data) {
        ...
        alert("Something went wrong while editing theme data");
      }
    };
    if (invokeAddTheme) {
  ...
  }, [addThemeData, close, editId, cookies, navigate, invokeAddTheme]);
  ...
  useEffect(() => {
    const getThemeDetail = async () => {
      ...
      if (response.customcode === 200) {
        ...
        setCoverPicturePreviewFile(response.data.url);
        setCoverPictureFile(response.data.url);
      } else if (response.customcode === 201) {
        navigate("/logout");
      } else {
        alert("Something went wront while getting theme details");
      }
    };
    if (invokeTheme) {
      setinvokeTheme(false);
      getThemeDetail();
    }
  }, [cookies, editId, navigate, invokeTheme]);
